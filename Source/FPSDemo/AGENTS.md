...
- 所有代码注释必须使用中文。
- 生成代码时，需要在必要的地方添加注释，说明关键逻辑、复杂流程或容易误解的实现意图。
- 修改代码时，如果碰到原有英文注释，需要同步翻译为中文。
- 与引擎或 API 强相关的专有术语可以保留英文，例如 `Blueprint`、`Actor`、`GameMode`、`HitResult`。

## C++ 文件格式规范

- 所有 C++ 源码文件（`.h`、`.cpp`）文件末尾必须保留一个空行，生成或修改代码时必须确保最后一行为空行，符合 UE 标准要求。
